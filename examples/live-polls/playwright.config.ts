...
export default defineConfig({
  ...
  webServer: process.env.AYB_DEMO_EXTERNAL_SERVER ? undefined : {
    command:
      ...
      `AYB_SERVER_PORT="$(pick_free_port 48090 49090 50090 51090 52090)" || { echo "No free isolated AYB API port available for Live Polls demo" >&2; exit 1; }; export AYB_SERVER_PORT; ` +
      `AYB_SERVER_URL="http://127.0.0.1:$AYB_SERVER_PORT"; export AYB_SERVER_URL; ` +
      `AYB_DATABASE_EMBEDDED_PORT="$(pick_free_port 45432 46432 47432 48432 49432)" || { echo "No free isolated embedded Postgres port available for Live Polls demo" >&2; exit 1; }; export AYB_DATABASE_EMBEDDED_PORT; ` +
      `AYB_DATABASE_EMBEDDED_DATA_DIR="$(mktemp -d /tmp/ayb-live-polls-pg.XXXXXX)" || { echo "Cannot create isolated embedded Postgres data directory for Live Polls demo" >&2; exit 1; }; export AYB_DATABASE_EMBEDDED_DATA_DIR; ` +
      `runtime_home="$(mktemp -d /tmp/ayb-live-polls-home.XXXXXX)" || { echo "Cannot create isolated home directory for Live Polls demo" >&2; exit 1; }; ` +
      ...
      `AYB_DEMO_APP_PORT=${port} ../../ayb demo live-polls & demo_pid="$!"; wait "$demo_pid"'`,
