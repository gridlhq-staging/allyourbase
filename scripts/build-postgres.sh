...
normalize_arch_token() {
  ...
  normalized_token="$(printf "%s" "$raw_token" | tr '[:upper:]' '[:lower:]')"
  ...
  case "$normalized_token" in
    ...
    aarch64|arm64|arm64e)
      ...
      ;;
    *)
      echo "Unsupported architecture token: $raw_token" >&2
      return 1
      ;;
  esac
}
...
verify_linux_postgres_architecture() {
  ...
  local install_dir="$2"
  local postgres_binary="$install_dir/bin/postgres"
  local file_output
  local detected_token
  local detected_arch

  if [ ! -f "$postgres_binary" ]; then
    echo "Missing postgres binary for architecture verification: $postgres_binary" >&2
    return 1
  fi

  file_output="$(file -b "$postgres_binary")"

  if ! printf "%s\n" "$file_output" | grep -Eqi '(^|[^[:alnum:]_])ELF([^[:alnum:]_]|$)'; then
    echo "Linux architecture verification failed: not an ELF binary (file_output=$file_output requested_arch=$requested_arch)" >&2
    return 1
  fi

  detected_token="$(printf "%s\n" "$file_output" | grep -Eo 'x86[-_]?64|amd64|aarch64|arm64' | head -n1 || true)"

  if [ -z "$detected_token" ]; then
    echo "Unable to detect linux postgres architecture token from file output: $file_output" >&2
    return 1
  fi

  detected_arch="$(normalize_arch_token "$detected_token")"
  if [ "$detected_arch" != "$requested_arch" ]; then
    echo "Architecture mismatch: detected_token=$detected_token detected_arch=$detected_arch requested_arch=$requested_arch" >&2
    return 1
  fi
}
...
main() {
  ...
  if [ "${TARGET_OS}" = "linux" ]; then
    verify_linux_postgres_architecture "${TARGET_ARCH}" "${INSTALL_DIR}"
  fi
