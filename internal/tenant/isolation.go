...
func NewSchemaProvisioner(pool *pgxpool.Pool, logger *slog.Logger) *SchemaProvisioner {
	if pool == nil {
		return newSchemaProvisioner(nil, logger)
	}
	return newSchemaProvisioner(pool, logger)
}

func newSchemaProvisioner(database schemaExecutor, logger *slog.Logger) *SchemaProvisioner {
...
func (p *SchemaProvisioner) ProvisionSchema(ctx context.Context, slug string) error {
	if p == nil || p.database == nil || slug == "" {
		return nil
	}

	schemaName := pgx.Identifier{slug}.Sanitize()
	if _, err := p.database.Exec(ctx, fmt.Sprintf(`CREATE SCHEMA IF NOT EXISTS %s`, schemaName)); err != nil {
		return fmt.Errorf("creating schema %q: %w", slug, err)
	}
	if _, err := p.database.Exec(ctx, fmt.Sprintf(`GRANT USAGE ON SCHEMA %s TO %s`, schemaName, rlsAuthenticatedRole)); err != nil {
...
func (p *SchemaProvisioner) DropSchema(ctx context.Context, slug string) error {
	...
	schemaName := pgx.Identifier{slug}.Sanitize()
	if _, err := p.database.Exec(ctx, fmt.Sprintf(`DROP SCHEMA IF EXISTS %s CASCADE`, schemaName)); err != nil {
