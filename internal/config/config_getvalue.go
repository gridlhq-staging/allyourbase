...
var configKeyRegistry = withConfigKeyMetadata(map[string]configKeyMetadata{
	...
	"server.require_resolved_tenant":             {getter: func(cfg *Config) any { return cfg.Server.RequireResolvedTenant }},
	...
	"auth.webauthn_enabled":                      {getter: func(cfg *Config) any { return cfg.Auth.WebAuthnEnabled }},
...
var boolCoercionKeys = []string{
	...
	"auth.totp_enabled",
	"auth.webauthn_enabled",
	"storage.enabled",
	"storage.s3_use_ssl",
	"server.tls_enabled",
	"server.tls_staging",
	"server.require_resolved_tenant",
