...
`ayb migrate supabase` connects directly to Supabase PostgreSQL and recreates tables, rows, views, admitted user-schema `plpgsql` and `sql` functions, and ordinary table triggers whose handlers were migrated; it also preserves email auth users plus raw user/app metadata JSONB, migrates non-email OAuth identity links, rewrites supported RLS policies, and copies storage from either a local export or direct S3 pull with normalized bucket names and registered metadata. The bounded function subset excludes excluded-schema body references, unsupported function languages and kinds (including C, `internal`, unknown, aggregate, window, procedure, and operator implementations), extension-owned functions, and signatures that depend on table/view composite types; unsupported trigger kinds (event and constraint triggers) and triggers without migrated handlers are also skipped. Phone-only users, MFA factors, secondary indexes, source grants, and full custom-type fidelity remain unsupported. Non-`public` schemas are created without access for `ayb_authenticated`; grant only the privileges the application requires after reviewing migrated RLS policies. The remaining storage residual is credential-gated live confirmation against `https://<project>.storage.supabase.co/storage/v1/s3`.
