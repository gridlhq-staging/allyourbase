...
func TestWingetManifestContainsRequiredKeys(t *testing.T) {
	...
	for _, arch := range []string{"x64", "arm64"} {
		...
		switch arch {
		case "x64":
			if want := "https://github.com/AllyourbaseHQ/allyourbase/releases/download/v0.0.0/ayb_0.0.0_windows_amd64.zip"; url != want {
				t.Fatalf("installer %s url = %q, want %q", arch, url, want)
			}
		case "arm64":
			if want := "https://github.com/AllyourbaseHQ/allyourbase/releases/download/v0.0.0/ayb_0.0.0_windows_arm64.zip"; url != want {
				t.Fatalf("installer %s url = %q, want %q", arch, url, want)
			}
		}
...
func TestScoopManifestContainsRequiredKeys(t *testing.T) {
	...
	requireStringEqual(t, requireMapKey(t, manifest, "homepage"), "homepage", "https://github.com/AllyourbaseHQ/allyourbase")
	requireStringEqual(t, requireMapKey(t, manifest, "license"), "license", "MIT")
	...
	requireStringEqual(t, requireMapKey(t, checkver, "github"), "checkver.github", "https://github.com/AllyourbaseHQ/allyourbase")
	...
	requireStringEqual(
		...
		"https://github.com/AllyourbaseHQ/allyourbase/releases/download/v$version/ayb_$version_windows_amd64.zip",
	)
	requireStringEqual(
		t,
		...
		"https://github.com/AllyourbaseHQ/allyourbase/releases/download/v$version/ayb_$version_windows_arm64.zip",
	)
}
...
func TestGoreleaserContainsScoopBucketConfig(t *testing.T) {
	...
	if owner != "AllyourbaseHQ" && owner != "gridlhq-staging" {
		t.Fatalf("scoop.repository.owner = %q, want AllyourbaseHQ or gridlhq-staging", owner)
	}
	...
	if homepage != "https://github.com/AllyourbaseHQ/allyourbase" &&
